function [ppieC]=D2Eq_Mome_S2_RBC(u_star,v_star,rho,mu,p,psi,phi,mcapXP,mcapXE,mcapXf,mcapYP,mcapYN,mcapYf,ppie)
global  imax imin jmax jmin dt nx ny dxi dyi dx dy RhieChow_or_Not gy gx epsilon 
global sigmafX sigmafY
% 中心差分计算通量+RhioChow修正
%%
%==================系数矩阵：▽·（1/ρ*▽P'）===========
 ppieC=zeros(imax+3,jmax+3);%容器
...
%===================Red_Black Coloring Solve==============================
%外部已给一次Ppie_BC
BUstar=1.5/dt*adv_Ustar; % b(U)
[rhoe,rhow,rhon,rhos] = D2Matrix_FaceMeanC(rho);  %系数  
rhoeT=1./rhoe;   rhowT=1./rhow;   rhonT=1./rhon;   rhosT=1./rhos;
a= -dxi^2 * (rhoeT+rhowT) - dyi^2  * (rhonT+rhosT);
Le=  dxi^2 * rhoeT;
Lw=  dxi^2 * rhowT;
Ln=  dyi^2 * rhonT;
Ls=  dyi^2 * rhosT;
%红
% ppie=ppie(imin:imax,jmin:jmax);

ppieE=circshift(ppie,-1,1);ppieE=ppieE(imin:imax,jmin:jmax);
ppieW=circshift(ppie, 1,1);ppieW=ppieW(imin:imax,jmin:jmax);
ppieN=circshift(ppie,-1,2);ppieN=ppieN(imin:imax,jmin:jmax);
ppieS=circshift(ppie,-1,2);ppieS=ppieS(imin:imax,jmin:jmax);
ppie=a.*( BUstar...
                 - Le.*ppieE...
                 - Lw.*ppieW...
                 - Ln.*ppieN...
                 - Ls.*ppieS);
ppie=ppie-ppie(nx,ny);
ppieC(imin:imax,jmin:jmax)=ppie;
