%  load('D2Data_Multy-RC1-128-Vis'); istep_max=10000;   % 续算时开启
...
switch initial_type   
    ...
    case 6 %自动开启 singel drop 定量对比的向量初始化        
        ...
        [uL,vL]=D2InitialUVdb(Coord_x,Coord_y,imax,imin,jmax,jmin);  %Drop breakup时开启   
    case 14
        [u,v]  =D2InitialUVhsl(Coord_x,Coord_y,imax,imin,jmax,jmin);   %horizontal shear layer
        [uL,vL]=D2InitialUVhsl(Coord_x,Coord_y,imax,imin,jmax,jmin); 
end
...
csi=0.8;   %强度
cs = csi * (1-abs(phi))+0.01;     % Concentrations of Surfactant Bulk浓度不为0，需考虑附加小值
cpsi=0*phi;%便于未考虑表面活性剂计算时，export2可以运行


% csL=cs ;%由phi计算得到，故不需BC
% contourf(Coord_x(4:1+nx),Coord_y(4:1+ny),cs(4:1+nx,4:1+ny)',3);axis equal;
