function [A,av]=D2Matrix_Av(rho,mu)
...
av.e=reshape(Ae(imin:imax,jmin:jmax),nx*ny,1);
av.w=reshape(Aw(imin:imax,jmin:jmax),nx*ny,1);
av.n=reshape(An(imin:imax,jmin:jmax),nx*ny,1);
av.s=reshape(As(imin:imax,jmin:jmax),nx*ny,1);
...
aee=av.e.*D2LocR.e; aww=av.w.*D2LocR.w;
ass=av.s.*D2LocR.s; ann=av.n.*D2LocR.n;
