function [ppie]=D2RhieChow(u_star,v_star,rho,mu,p,psi,phi,mcapXP,mcapXE,mcapXf,mcapYP,mcapYN,mcapYf,sigma)
global  imax imin jmax jmin dt nx ny dxi dyi dx dy RhieChow_or_Not  epsilon
...
%=========Fs修正，=ψδφ
CoffFsCorrect=0.75*sqrt(2)/epsilon;
[PsiX,PsiY] = D2Matrix_FaceMeanF(psi) ;
[sigma] = D2set_BCNeu(sigma);
[Sigmafx,Sigmafy]= D2Matrix_FaceMeanF(sigma) ;
GradxPhi=D2Gradx_Matrix(phi);GradyPhi=D2Grady_Matrix(phi);
FsXCorrect= CoffFsCorrect*dx_fCap ...
    .* ( PsiX .*Sigmafx.*(phi(imin:imax+1,jmin:jmax)-phi(imin-1:imax,jmin:jmax))/dx  ...
    -0.5*Rhox_f.*(psi(imin-1:imax,jmin:jmax) .*Sigmafx.* GradxPhi(imin-1:imax,jmin:jmax) ./ rho(imin-1:imax , jmin:jmax)...
    +psi(imin:imax+1,jmin:jmax) .*Sigmafx.* GradxPhi(imin:imax+1,jmin:jmax) ./ rho(imin:imax+1 , jmin:jmax) )) ;
FsYCorrect= CoffFsCorrect*dy_fCap ...
    .* ( PsiY .*Sigmafy.*(phi(imin:imax,jmin:jmax+1)-phi(imin:imax,jmin-1:jmax))/dy  ...
    -0.5*Rhoy_f.*(psi(imin:imax,jmin-1:jmax) .*Sigmafy.* GradyPhi(imin:imax,jmin-1:jmax) ./ rho(imin:imax , jmin-1:jmax)...
    +psi(imin:imax,jmin:jmax+1) .*Sigmafy.* GradyPhi(imin:imax,jmin:jmax+1) ./ rho(imin:imax , jmin:jmax+1) )) ;
...
% 1 系数为dCap 或者 dt
% 2 压力多项式是否加入了密度修正
% 3 是否考虑了mcap和Q（包含fext，即外力修正）
% 4 Fs如何表示和修正，是否包含系数6sqrt（2）
...
switch RhieChow_or_Not
    ...
    case 6 %自导dcap + 密度基修正δP + 表面张力修正
        ...
        FaceUX_Correct=  (0.25/dx*dx_fCap)...
            ...
            +ax.E .*p(imin+1-1:imax+2-1   , jmin:jmax)...
            +ax.EE .*p(imin+2-1:imax+3-1 , jmin:jmax));
        FaceUX_RC2=FaceUX+FaceUX_Correct+FsXCorrect;
        ...
        FaceVY_Correct=  (0.25/dy*dy_fCap)...
            ...
            +ay.N .*p(imin:imax ,jmin+1-1:jmax+2-1  )...
            +ay.NN .*p(imin:imax ,jmin+2-1:jmax+3-1));
        FaceVY_RC2=FaceVY+FaceVY_Correct+ FsYCorrect;
        adv_Ustar= dxi*( FaceUX_RC2(2:end,:)-FaceUX_RC2(1:end-1,:))...
            +dyi*(FaceVY_RC2(:,2:end)-FaceVY_RC2(:,1:end-1))  ;
        
    case 7 %自导dcap + 密度基修正δP + 表面张力修正 +瞬态项修正
        [FaceUX,~] = D2Matrix_FaceMeanF(u_star) ;
        FaceUX_Correct=  (0.25/dx*dx_fCap)...
            .* (ax.W .*p(imin-1-1:imax-1 , jmin:jmax)...
            +ax.P .*p(imin-1  :imax+1-1 , jmin:jmax)...
            +ax.E .*p(imin+1-1:imax+2-1   , jmin:jmax)...
            +ax.EE .*p(imin+2-1:imax+3-1 , jmin:jmax));
        FaceUX_RC2=FaceUX+FaceUX_Correct+FsXCorrect+mcapX_correction;
        ...
        FaceVY_Correct=  (0.25/dy*dy_fCap)...
            ...
            +ay.N .*p(imin:imax ,jmin+1-1:jmax+2-1  )...
            +ay.NN .*p(imin:imax ,jmin+2-1:jmax+3-1));
        FaceVY_RC2=FaceVY+FaceVY_Correct+FsYCorrect+mcapY_correction;
        adv_Ustar= dxi*( FaceUX_RC2(2:end,:)-FaceUX_RC2(1:end-1,:))...
            +dyi*(FaceVY_RC2(:,2:end)-FaceVY_RC2(:,1:end-1))  ;
end
