...
plot(SEM1000Vc(:,1),SEM1000Vc(:,2),'bx');
% plot(Hysing(:,1),Hysing(:,2),'r^');
legend('WENO-FVM,Cn=0.02','SEM,Cn=0.02','Hysing','location','southeast');
title("Rising bubble Ratio=1000 ");
axis([0,3,0,0.25]) ;

subplot(224)
%%  YC
plot(Time,Yc,'-k');hold on
plot(SEM1000Yc(:,1),SEM1000Yc(:,2),'bx');
% load 'HysingYC.mat'
...
plot(SEM1000Shape(:,1),SEM1000Shape(:,2),'bx');%谱元
% load('HysingShape10.mat');
% scatter(HysingShape(:,1),HysingShape(:,2),'r^');
% hold on
legend('WENO-FVM,Cn=0.01','SEM,Cn=0.02','Hysing','location','northwest');
axis equal;
axis([0.1,0.5,0.5,1.8]) ;
